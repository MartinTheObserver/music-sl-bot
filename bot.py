...
class ZoneSelect(View):
    def __init__(self, locations):
        ...
        options = []
        for loc in locations[:25]:  # Discord max options per select
            city = loc["name"]
            region = loc.get("adminName1")
            country = loc["countryName"]
            label = f"{city}, {region}, {country}" if region else f"{city}, {country}"
            value = f"{loc['lat']},{loc['lng']}"
            options.append(discord.SelectOption(label=label, value=value))

        self.select = discord.ui.Select(
            placeholder="Select your location",
            options=options,
            custom_id="zone_select"
        )
        self.select.callback = self.select_callback
        self.add_item(self.select)

    async def select_callback(self, interaction: discord.Interaction):
        lat, lng = map(float, self.select.values[0].split(","))
        tz = get_timezone(lat, lng)
        label = self.select.options[self.select.values.index(self.select.values[0])].label
        await interaction.response.edit_message(content=f"🌍 {label} → `{tz}`", view=None)
...
async def ecm(ctx):
    ...
    embed.add_field(name="!zone <location>", value="Discover your timezone", inline=False)
    await ctx.send(embed=embed)
