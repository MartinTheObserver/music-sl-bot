...
API_NINJAS_KEY = os.getenv("API_NINJAS_KEY")  # For random word feature
...
class WordView(View):
    ...
    async def generate(self):
        ...
        for c in chunk(defs, 5):
            self.pages.append(build_embed(word, f"[Definitions](https://en.wiktionary.org/wiki/{word})", "\n".join(f"• {d}" for d in c)))
        for c in chunk(examples, 4):
            self.pages.append(build_embed(word, f"[Examples](https://en.wiktionary.org/wiki/{word})", "\n".join(f"• {e}" for e in c)))
        for c in chunk(rel, 8):
            related_str = ", ".join(c) if isinstance(c, list) else str(c)
            self.pages.append(build_embed(word, f"[Related Words](https://api.datamuse.com/words?ml={word})", related_str))
        self.pages.append(build_embed(word, f"[Etymology](https://en.wiktionary.org/wiki/{word}#Etymology)", ety[:900]))
        self.index = 0

    async def get_random_word(self):
        try:
            headers = {"X-Api-Key": API_NINJAS_KEY}
            r = requests.get("https://api.api-ninjas.com/v1/randomword", headers=headers, timeout=10)
            r.raise_for_status()
            data = r.json()
            return data.get("word", "Error")
        except Exception as e:
            return f"Error: {e}"
...
# Slash Command: /word
# ---------------------------
@tree.command(name="word", description="Get a random word and info")
